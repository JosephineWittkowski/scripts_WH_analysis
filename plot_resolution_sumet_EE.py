...
inputfiles = [ROOT.TFile("/data/etp/jwittkowski/analysis_SUSYTools_03_04/histo_Zee_pureMETxy_10GeVZwindow_data_variableBinning.root"),
...
for i_f, inputfile in enumerate(inputfiles):	     
  f = open("parameters_Zee_" + data_type[i_f] + "_varBins.txt", 'w')
  for i_jet in range(0,4):
    ...
    histoname_metxy = "h_metxy_sumet_" + str(i_jet) + "j_EE"
    c1 = ROOT.TCanvas(histoname_metxy, histoname_metxy,  600, 600)
    print histoname_metxy
    #histoname_metxy_SS = "h_metxy_sumet_" + str(i_jet) + "j_SS_EE"
    ...
    ROOT.gStyle.SetOptStat()
    c1.SaveAs("../pics/res_met_"  + str(i_jet) + "j_Zee.pdf")
	...
	text1.Draw("same")
      print data_type[i_f]
      c1.SaveAs("../pics/gausfit_Zee_" + str(i_bin) + "_" + str(i_jet) + "j_" + data_type[i_f] + "_varBins.pdf")
    c1 = ROOT.TCanvas("TGraph", "TGraph", 600, 600)
    graph.Draw("ALP")
    graph.SetTitle("Zee_"+ str(i_jet) + "j_" + data_type[i_f] + "_varBins")
    ...
    fit.Draw("same")
    ROOT.gStyle.SetOptStat()
    c1.SaveAs("../pics/gausfit_Zee_profile_" + str(i_jet) + "j_" + data_type[i_f] + "_varBins.pdf")
