...
name_of_variables = ["HT"]#, "pTl1", "mll", "METrel", "mWWt", "Mljj",  "mTlmax", "mTlmin", "DeltaEtall", "Mlj", "mT2J", "HT", "mT2", "etal0", "etal1"
#]
...
x_axis_title = ["HT", "pTl1", "mll", "METrel", "mWWt", "Mljj",  "mTlmax", "mTlmin", "DeltaEtall", "Mlj", "mT2J", "HT", "mT2", "etal0", "etal1"
...
mcid_of_grid_point = ["177501", "177502", "177503"]
...
if print_numbers_contributions:
  ...
  for i_rf, rf in enumerate(rootfiles_cutflow):  
    ...
    bg_histo_MM = rf.Get("cutflow_MM")	
    bg_histo_projected_MM = bg_histo_MM#.ProjectionX("cutflow_MM" + name_of_contribution[i_rf], 2, 2)
    bg_histo_EM = rf.Get("cutflow_EM")	
    bg_histo_projected_EM = bg_histo_EM#.ProjectionX("cutflow_EM" + name_of_contribution[i_rf], 2, 2)
    #bg_histo_total = rf.Get("cutflow_MM")	
    if name_of_contributionLegend[i_rf]!= "126893":
      sum_EE_1j = sum_EE_1j + bg_histo_projected_EE.Integral(34, 34)
      sum_MM_1j = sum_MM_1j + bg_histo_projected_MM.Integral(32, 32)
      sum_EM_1j = sum_EM_1j + bg_histo_projected_EM.Integral(32, 32)
      sum_EE_2j = sum_EE_2j + bg_histo_projected_EE.Integral(42, 42)
      sum_MM_2j = sum_MM_2j + bg_histo_projected_MM.Integral(41, 41)
      sum_EM_2j = sum_EM_2j + bg_histo_projected_EM.Integral(41, 41)
    #sum_total = sum_total + bg_histo_total.GetBinContent( 1)
    print '{\\tiny ', name_of_contributionLegend[i_rf], ":} & {\\tiny ",round(bg_histo_projected_EE.Integral(34, 34),2), "} & {\\tiny ", round(bg_histo_projected_MM.Integral(32, 32),2), "} & {\\tiny ",round(bg_histo_projected_EM.Integral(32, 32),2), "} & {\\tiny ",round(bg_histo_projected_EE.Integral(42, 42),2), "} & {\\tiny ", round(bg_histo_projected_MM.Integral(41, 41),2), "} & {\\tiny ",round(bg_histo_projected_EM.Integral(41, 41),2), "}\\\\"
  print "\hline"  
  print "\hline"  
...
for i_v, v in enumerate(name_of_variables):
  ...
  for i_s, s in enumerate(name_of_SR):
    ...
    c1 = ROOT.TCanvas("c_" + v + "_" + s, "c_" + v + "_" + s, 600, 600)
    ...
    grid_point_file_histo_projected = []
    for i_g, grid_point_file in enumerate(rootfiles_gridpoints):
      ...
      grid_point_file_histo_projection = grid_point_file_histo.ProjectionX(histoname + "_1D_signal_" + name_of_contribution[i_rf] + str(i_g), ybinProj_list[i_s], ybinProj_list[i_s])
      grid_point_file_histo_projection.Rebin(nbins/binning)
      
      #make sure to use the same binning:
      
      #print "signal masspoint ", mcid_of_grid_point[i_s]
      #print "integral", round(grid_point_file_histo_projection_projected[i_g].Integral(),1)
      
      #find a factor for scaling signal to make it visible:
      signal_max = grid_point_file_histo_projection.GetMaximum()
	...
	scale = 10
      grid_point_file_histo_projection.Scale(scale)
      
      
      #draw signal in same TPad as stack:
      grid_point_file_histo_projection.SetLineColor(fillColors[i_g])
      grid_point_file_histo_projection.SetMarkerColor(fillColors[i_g])
      grid_point_file_histo_projection.SetMarkerSize(0.5)
      grid_point_file_histo_projection.SetLineWidth(2)
      grid_point_file_histo_projection.SetMarkerStyle(20)
      grid_point_file_histo_projected.append(grid_point_file_histo_projection)
      
      if draw_pad2 and i_g == 0:
	...
	h_ZN.SetLineStyle(2)
	h_ZN.SetTitle("")
	h_ZN.GetYaxis().SetLabelSize(0.2)
	h_ZN.Draw("l")
	...
	line_diff_eff = ROOT.TLine(pos_line_diff_eff_max ,0. , pos_line_diff_eff_max, Z_N_max)
    ...
    #print grid_point_file_histo_projected      
    for i_g, grid_point_file in enumerate(rootfiles_gridpoints):    
      pad1.cd()
      histo_signal = grid_point_file_histo_projected[i_g]
      #histo_signal.Print()
      histo_signal.Draw("hist same")
	...
	#line.Draw("same")
      ##if (i_g == 0):
      leg_stack.AddEntry(grid_point_file_histo_projected[i_g],str(mcid_of_grid_point[i_g]))
	...
	#end draw_pad2      
    #c1.SaveAs("/data/etp/jwittkowski/pics_reoptimization/cumulative_pdf_" + v + "_" + s + "_severalMassPoints_bin" + str(ybinProj_list[i_s]) + ".pdf")
    #c1.SaveAs("/data/etp/jwittkowski/int_note_whss/figures/signalregions/EM/EM_23j_pTl0_baselineSel_3rdLepVeto.eps")
